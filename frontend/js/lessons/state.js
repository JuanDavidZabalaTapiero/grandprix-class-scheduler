...
export function validateSlot(lesson, occupiedVehicles = [], options = {}) {
  ...
  // DB
  if (
    occupiedVehicles.includes(Number(lesson.vehicle_id)) &&
    Number(lesson.vehicle_id) !== Number(ignoreVehicleId)
  ) {
    return "duplicate-db";
  }

  // LOCAL
  const existsSameSlot = selectedLessons.some(
    (l) =>
      l.date === lesson.date &&
      l.hour === lesson.hour &&
...
export function getEditedLessons() {
  return Object.entries(editedLessons).map(([id, data]) => ({
    id: Number(id),
    ...data,
  })); // ARRAY
}
