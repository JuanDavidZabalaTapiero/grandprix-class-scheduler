    ...
    row.instructors.forEach((item) => {
      ...
      if (item.lesson) {
        ...
        lessonTd.addEventListener("click", () => {
          handleRegisteredClick({ lesson: item.lesson });
        });
        ...
        if (
          ...
          changeMode.selection.origin.id === item.lesson.id
        ) {
        ...
        if (
          changeMode.active &&
          changeMode.selection.target === item.lesson.id
        ) {
